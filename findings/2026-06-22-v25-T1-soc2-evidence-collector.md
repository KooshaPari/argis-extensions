...
SOC2 Type II audits require **12 months of continuous control evidence**.
Without automation, audit prep = 200+ hours of manual artifact collection per
quarter per repo × 119 repos. With automation: ~30 min/quarter.
...
- **`scripts/soc2-evidence-collector.py`** (370 LOC, stdlib-only)
  - 18 controls (CC1.1 through CC9.2)
  - Per-repo evidence: file path + sha256 + git head/branch + timestamp
...
- **`.github/workflows/soc2-evidence.yml`** (CI gate)
  ...
  - Uploads `soc2-evidence.json` + `soc2-summary.md` as build artifacts
