#!/usr/bin/env python3
"""SOC2 evidence collector for fleet repos.

Pillar L51 (SOC2 evidence automation) cycle-15 T1.
Collects evidence for CC1-CC9 controls and emits JSON to stdout or file.

Usage:
    python3 scripts/soc2-evidence-collector.py --control CC6.1 --output evidence.json
    python3 scripts/soc2-evidence-collector.py --all --output fleet-evidence-$(date +%F).json
"""
...
CONTROLS = {
    ...
    "CC2.3": {"name": "SSOT per repo", "check": lambda r: (r / "SSOT.md").exists() or any((r / "docs" / "ssot.md").exists() for _ in [0])},
    ...
    "CC6.8": {"name": "Malware scanning", "check": lambda r: any((r / ".github" / "workflows").glob("*.yml") and "secret-scan" in p.name.lower() for p in (r / ".github" / "workflows").iterdir()) if (r / ".github" / "workflows").exists() else False},
    "CC7.1": {"name": "Vulnerability management", "check": lambda r: any((r / ".github" / "workflows").glob("*.yml") and ("audit" in p.name.lower() or "deny" in p.name.lower()) for p in (r / ".github" / "workflows").iterdir()) if (r / ".github" / "workflows").exists() else False},
    "CC7.2": {"name": "Monitoring (OTel)", "check": lambda r: (r / "pheno-tracing").exists() or any("pheno-otel" in str(p) for p in r.iterdir())},
    ...
    "CC9.1": {"name": "Risk mitigation", "check": lambda r: any((r / "deny.toml").exists() or (r / ".github" / "workflows").glob("*.yml") for _ in [0])},
...
def collect(repo: Path, control_id: str) -> dict:
    ...
    try:
        satisfied = ctl["check"](repo)
    except Exception as exc:
        satisfied = False
        evidence["check_error"] = str(exc)
    evidence["satisfied"] = bool(satisfied)
    for candidate in (
        repo / f"{control_id}.md", repo / "docs" / f"{control_id}.md",
        repo / "AGENTS.md", repo / "SSOT.md", repo / "CODE_OF_CONDUCT.md",
        repo / "CHANGELOG.md", repo / "SECURITY.md", repo / "justfile",
        repo / ".pre-commit-config.yaml", repo / ".github" / "CODEOWNERS",
        repo / ".devcontainer" / "devcontainer.json", repo / "deny.toml",
    ):
        if candidate.exists() and candidate.is_file():
            evidence["artifact"] = str(candidate.relative_to(REPO_ROOT))
            evidence["artifact_sha256_12"] = _hash_file(candidate)
            break
    info = _git(repo)
...
def collect_all(controls: list[str] | None = None) -> dict:
    ...
    repos = [p for p in repos if (p / ".git").exists() or p.name in {"AgilePlus","HexaKit","PhenoCompose","phenodag","phenoShared","PhenotypeHandoff"}]
    fleet = []
    for r in repos:
        repo_ev = []
        for cid in controls:
            repo_ev.append(collect(r, cid))
        fleet.append({"repo": r.name, "controls": repo_ev})
    return {
        "schema_version": "1.0",
...
def render_summary(report: dict) -> str:
    lines = [f"# SOC2 Evidence Report — {report['generated_at']}", ""]
    lines.append(f"Framework: {report['framework']}  Repo root: `{report['repo_root']}`")
    lines.append(f"Repos scanned: {len(report['fleet'])}  Controls: {len(report['controls'])}")
    lines.append("")
    lines.append("## Coverage Matrix")
    lines.append("")
    lines.append("| Repo | " + " | ".join(c["id"] for c in report["controls"]) + " |")
    lines.append("|" + "---|" * (len(report["controls"]) + 1))
    for entry in report["fleet"]:
        cells = []
        for c in entry["controls"]:
            cells.append("Y" if c["satisfied"] else ".")
...
def main() -> int:
    ...
    ap.add_argument("--control", help="Single control id (e.g. CC6.1)")
    ap.add_argument("--all", action="store_true", help="All 18 controls")
    ...
    if not args.control and not args.all:
        ap.error("specify --control CCx.x or --all")
