...
impl StateStore {
    ...
    /// after `webhook::deliver_all` returns a non-success report. The row
    /// is the seed for meta-alert evaluation (slice 18).
    ...
    /// `window_secs` ending at `now_unix`. Used by meta-alert evaluation to
    /// decide if `consecutive_failures` has been reached.
    pub fn count_failures_in_window(
        ...
        // window_secs and now_unix both feed the lower bound `now - window`.
    ...
    /// Returns the number of rows deleted. u64 -> i64 cast is saturated
    /// per the slice 10b pattern.
